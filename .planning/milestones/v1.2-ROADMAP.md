...
**Success Criteria:**
  1. `TorchKeeper/Models/GearItem.cs` and `MagicItem.cs` have `IsFreeCarry` property
  2. `TorchKeeper/DTOs/CharacterSaveData.cs` `GearItemData` and `MagicItemData` include `IsFreeCarry`
  3. `TorchKeeper/Services/CharacterFileService.cs` `MapToDto`/`MapFromDto` project `IsFreeCarry` correctly
